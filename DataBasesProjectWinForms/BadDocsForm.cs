...
{
    public partial class BadDocsForm : Form
    {
        private string sqlConnectionString = Config.sqlConnectionString;

        DataGridView docsTable;

        private void BadDocsForm_Load(object? sender, EventArgs e)
        {
            ...
            this.docsTable.Columns.Add("deaths", "Кол-во умерших пациентов");
            ...
            using (var connection = new SqliteConnection(sqlConnectionString))
            {
                ...
                docCommand.CommandText = "SELECT id, Name, Surname, DepartmentId FROM Doctor WHERE FireDate IS NULL";
                ...
                for (int i = 0, id = 0; i < this.docsTable.Rows.Count; ++i)
                {
                    ...
                    if (deaths.ContainsKey(id))
                    {
                        if (deaths[id] == 0)
                        {
                            this.docsTable.Rows.Remove(this.docsTable.Rows[i]);
                            i -= 1;
                        }
                        else
                        {
                            this.docsTable.Rows[i].Cells["deaths"].Value = deaths[id];
                        }
                    }
                }
